...
function add_game(info) {
  ...
  new_card.addEventListener('click', () => {
    ...
    disable_patch_game_button.addEventListener("click", () => {
      disable_patch_game_button.classList.add("button-loading");
      disable_patch_game_button.style.animation = "button-to-loading 1s ease forwards, rotate 5s infinite 500ms linear";
      invoke("disable_patch", { globalId: info["global_id"] }).then(() => {
        setTimeout(() => {
          disable_patch_game_button.style.animation = "button-to-loading-reversed 1s ease forwards";
          disable_patch_game_button.classList.remove("button-loading");
